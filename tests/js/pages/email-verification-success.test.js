 ...
 * AuthKit
 * -----------------------------------------------------------------------------
 * File: tests/js/pages/email-verification-success.test.js
 * Author: Michael Erastus
 * Package: AuthKit
 *
 * Description:
 * -----------------------------------------------------------------------------
 * Unit tests for the AuthKit email verification success page runtime module.
 *
 * Responsibilities:
 * - Verify email-verification-success page boot eligibility.
...
import {
    boot,
    getEmailVerificationSuccessPageElements,
...
/**
 * Build a minimal AuthKit runtime context for email verification success
 * page-module testing.
 *
 * @param {Object} [overrides={}]
 * @returns {Object}
 */
function createEmailVerificationSuccessTestContext(overrides = {}) {
...
describe('pages/email-verification-success', () => {
    beforeEach(() => {
    ...
    describe('form discovery', () => {
        it('returns an empty forms collection when no form exists on the success page', () => {
            ...
            const context = createEmailVerificationSuccessTestContext();
    ...
    describe('page element discovery', () => {
        it('builds a normalized email verification success page descriptor from rendered markup', () => {
            document.body.innerHTML = `
                <main class="authkit-page authkit-auth-page" data-authkit-page="email_verification_success">
                    <div class="authkit-page-container">
                        <div class="authkit-card">
                            <div class="ak-alert">Email verified successfully.</div>
                            <a href="/login">Continue to login</a>
                        </div>
                    </div>
                </main>
    ...
    describe('boot', () => {
        it('boots successfully on the email verification success page', () => {
            document.body.innerHTML = `
                <main class="authkit-page authkit-auth-page" data-authkit-page="email_verification_success">
                    <a href="/login">Continue to login</a>
                </main>
            ...
            expect(result?.key).toBe('email_verification_success');
            expect(result?.formCount).toBe(0);
            expect(result?.links).toHaveLength(1);
            expect(result?.primaryActionLink?.getAttribute('href')).toBe('/login');
        });

        it('returns null when the current page is not the email verification success page', () => {
            document.body.innerHTML = `
        ...
        it('returns null when the supplied context is invalid', () => {
            document.body.innerHTML = `
                <main class="authkit-page authkit-auth-page" data-authkit-page="email_verification_success"></main>
            `;
